const PAGE_FILE =
  (window.__RUNTIME_PAGE_FILE__ || window.location.pathname.split("/").pop() || "guia3.html").toLowerCase();
const STORAGE_FILE_ALIASES = {
  ...
  "plantilla-grado-10-guia-03-planificar-informacion.html": "grado_guia3.html",
};
const GUIDE_DATA_FILE = STORAGE_FILE_ALIASES[PAGE_FILE] || PAGE_FILE;
const PAGE_KEY = GUIDE_DATA_FILE.replace(/[^a-z0-9]+/g, "_") || "guia3";
...
const supportBundle = [
  {
    ...
    type: "ZIP local",
    description:
      "Compilado de documentos y recursos complementarios de la Guia 3 para consulta o respaldo.",
...
const GUIDE3_DRIVE_ACTIVITY_TARGETS = [
  {
    activityNumber: "3.1.1",
    panelKey: "guide3-3-1-1",
    ...
    activityContext: {
      ...
      fileNamePrefix: "Bitacora_Guia3_EstudioCaso",
      learnerNameMode: "full",
    },
  },
  {
    activityNumber: "3.2.1",
    panelKey: "guide3-3-2-1",
    ...
    activityContext: {
      ...
      fileNamePrefix: "TablaResumen_Guia3_Bloque",
      learnerNameMode: "full",
    },
  },
  {
    activityNumber: "3.2.2",
    panelKey: "guide3-3-2-2",
    ...
    activityContext: {
      ...
      fileNamePrefix: "MapaConceptual_Guia3",
      learnerNameMode: "full",
    },
  },
  {
    activityNumber: "3.3.1",
    panelKey: "guide3-3-3-1",
    ...
    activityContext: {
      ...
      fileNamePrefix: "Checklist_Guia3_Instalacion",
      learnerNameMode: "full",
    },
  },
  {
    activityNumber: "3.3.2",
    panelKey: "guide3-3-3-2",
    ...
    activityContext: {
      ...
      fileNamePrefix: "Diagnostico_Guia3",
      learnerNameMode: "full",
    },
  },
  {
...
if (!window.__PAGE_CONTEXT__) {
  document.addEventListener("DOMContentLoaded", initGuia3);
}
...
function isValidEquipoGuia3Team(raw) {
  ...
  if (raw.mode === "solo" || raw.solo === true) {
    return Array.isArray(raw.members) && raw.members.length >= 1;
  }
...
function renderEquipoGuia3EmptyCta(activityId) {
  ...
  summary.innerHTML = `
    <div style="padding:10px 12px;background:#fffbe6;border:1px solid #f3e0a3;border-left:4px solid #c79a00;border-radius:6px;color:#5a4500;font-size:.86rem">
      <div style="margin-bottom:8px">Configura tu equipo para esta actividad, o continua de forma individual.</div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
        ...
        <button type="button" onclick="trabajarSoloGuia3('${escapeHtml(activityId)}')" style="display:inline-flex;align-items:center;gap:6px;padding:8px 14px;border:1px solid #b6dba0;background:#e8f5e9;color:#1b5e20;border-radius:6px;font-family:inherit;font-size:.86rem;cursor:pointer;font-weight:600">
          &#128100; Trabajar solo
        </button>
      </div>
      <div style="margin-top:6px;font-size:.78rem;color:#7a6500">Si no aparecen tus compa&ntilde;eros o prefieres hacerlo individual, pulsa "Trabajar solo" y continua.</div>
    </div>`;
}
...
function renderEquipoGuia3Summary(activityId, team) {
  ...
  summary.innerHTML = `
    <div style="padding:10px 12px;background:#e8f5e9;border:1px solid #b6dba0;border-left:4px solid #2e7d32;border-radius:6px;color:#1b5e20;font-size:.88rem">
      <div style="font-weight:700;margin-bottom:4px">${(team.solo || team.mode === "solo") ? "&#128100; Trabajando de forma individual" : "Equipo confirmado"} &middot; <code style="background:#fff;padding:2px 6px;border-radius:4px;color:#0b6b35">${(team.solo || team.mode === "solo") ? "Individual" : "Equipo " + escapeHtml(code4)}</code></div>
...
function renderEquipoGuia3Roster(activityId) {
  ...
  if (rosterList.length === 0) {
    mount.innerHTML = `
      <p style="margin:0 0 8px;font-size:.86rem;color:#a13029">Aun no aparecen companeros registrados en la ficha ${escapeHtml(ficha)} (pueden tardar en cargar). Pideles que inicien sesion, o continua de forma individual.</p>
      <button type="button" onclick="trabajarSoloGuia3('${escapeHtml(activityId)}')" style="display:inline-flex;align-items:center;gap:6px;padding:8px 14px;border:1px solid #b6dba0;background:#e8f5e9;color:#1b5e20;border-radius:6px;font-family:inherit;font-size:.86rem;cursor:pointer;font-weight:600">&#128100; Trabajar solo</button>`;
...
async function trabajarSoloGuia3(activityId) {
  ...
  const memberKeys = allMembers.map((m) => m.usernameKey);
  const memberEmails = memberKeys.map((k) => k + EQUIPO_GUIA3_EMAIL_DOMAIN);
  const groupKey = buildEquipoGuia3GroupKey(ficha, memberKeys.concat([activityId, "solo"]));
  const team = {
    mode: "solo",
    solo: true,
    groupKey,
...
window.trabajarSoloGuia3 = trabajarSoloGuia3;
...
function buildInstitutionalWordHeader(title, learnerName, selection, fecha) {
  ...
  <table class="institutional-header" width="100%" align="left" style="width:100%;margin-left:0;margin-right:0;">
    <tr><td class="label">Programa</td><td>${escapeWordValue(GUIDE3_WORD_METADATA.program)}</td></tr>
    <tr><td class="label">Fecha de elaboración</td><td>${escapeWordValue(fecha)}</td></tr>
    <tr><td class="label">Guía</td><td>${escapeWordValue(GUIDE3_WORD_METADATA.guideName)}</td></tr>
    <tr><td class="label">Actividad</td><td>${escapeWordValue(title)}</td></tr>
    <tr><td class="label">Competencia</td><td>${escapeWordValue(GUIDE3_WORD_METADATA.competencia)}</td></tr>
    <tr><td class="label">Resultado de aprendizaje</td><td>${escapeWordValue(GUIDE3_WORD_METADATA.resultado)}</td></tr>
...
const QUIZ_PAGE_URLS = {
  "grupo-10a-guia-03-planificar-informacion.html": "pages/auxiliares/grupo-10a-guia-03-herramientas-quiz.html",
...
function renderQuizHerramientasStatus() {
  ...
  const attempt = state?.["quiz-guia3-312"];
  if (!panel) return;
...
(function mountAvancePanelGuia3() {
  function render() {
