...
test("normalize: quita acentos y la tilde de la ñ; minusculas", () => {
  assert.equal(na.normalize("MONTAÑA Leynor"), "montana leynor");
  assert.equal(na.normalize("José Andrés Muñoz"), "jose andres munoz");
});

test("wordSetKey: orden, mayusculas y acentos no importan", () => {
  assert.equal(
    na.wordSetKey("BELTRAN PINILLA JHON DEIBY"),
    na.wordSetKey("Jhon Deiby Beltran Pinilla")
  );
  assert.equal(na.wordSetKey("PIÑEROS PINILLA"), na.wordSetKey("pineros pinilla"));
});
...
test("match: SEGURO con coincidencia unica de palabras y nombre distinto", () => {
  const users = [
    { usernameKey: "a", username: "a", fullName: "jhon deiby beltran pinilla", ficha: "3441944" },
    { usernameKey: "b", username: "b", fullName: "Ashlie Sofia Corredor Bernal", ficha: "3441944" },
  ];
  const res = na.matchOfficialNames(
    ["BELTRAN PINILLA JHON DEIBY", "CORREDOR BERNAL ASHLIE SOFIA"],
    users
  );
  assert.equal(res.sure.length, 2);
  assert.equal(res.sure[0].official, "BELTRAN PINILLA JHON DEIBY");
...
test("match: SIN CAMBIO cuando el nombre ya es identico", () => {
  const users = [{ usernameKey: "a", username: "a", fullName: "BELTRAN PINILLA JHON DEIBY", ficha: "1" }];
  const res = na.matchOfficialNames(["BELTRAN PINILLA JHON DEIBY"], users);
...
test("match: DUDOSO cuando varios aprendices coinciden (posible duplicado)", () => {
  const users = [
    { usernameKey: "a", username: "a", fullName: "Jhon Deiby Beltran Pinilla", ficha: "1" },
    { usernameKey: "b", username: "b", fullName: "Beltran Pinilla Jhon Deiby", ficha: "1" },
  ];
  const res = na.matchOfficialNames(["BELTRAN PINILLA JHON DEIBY"], users);
  assert.equal(res.sure.length, 0);
  assert.equal(res.uncertain.length, 1);
